...
function ucitajPorodicne() {
    var sveIgre = dohvatiIgre();

    porodicne = sveIgre.filter(igra => igra.kategorija === "Porodične");

    prikaziPorodicne(porodicne);
}
...
function prikaziPorodicne(igre) {
    ...
    if (igre.length === 0) {
        row.innerHTML = `
            <p class="ml-3">
                Nema igara koje odgovaraju pretrazi.
            </p>
        `;
        return;
    }
...
function filtrirajISortiraj() {
    ...
    var rezultat = porodicne.filter(igra => {
...
function generisiPDF(igre) {
    ...
    pdf.text("Katalog porodicnih igara", 20, y);
    ...
    pdf.save("katalog-porodicne-igre.pdf");
}
